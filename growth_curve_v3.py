...
# edited by Marco to improve lag time calculation and denoising data (which helps calculating saturation and max OD)
...
class Experiment(object):
    ...
    def _denoise(self, data, n):
        """
        Denoise the DataFrame by setting the reminder of rows in a window of n consecutive rows
        to the last value of the window if values are in a range of +- 0.2.
        ...
        Returns:
        - denoised_data: pandas DataFrame
        """

        denoised_data = data.copy()

        for col in denoised_data.columns[1:]:  # Assuming you want to start from the second column
            for i in range(len(denoised_data) - n):
                window = denoised_data[col].iloc[i:i+n]
                if np.all((window - window.iloc[0]).abs() <= 0.05):
                    denoised_data.loc[i+n:, col] = window.iloc[-1]
        print ('data denoised')
        return denoised_data

    def __init__(self, path_to_data, plate_layout=None, blank=None, blank_file=None, out_dir='./', window_size=9,
                 correction=None, denoising=0):
        self.path_to_data = path_to_data
        self.denoising = denoising  # Save denoise_window as an attribute
        ...
        if denoising > 0:
            self.data = self._denoise(self.data, denoising)
        self.samples = {}
    ...
    def analyze_sample_data(self, method='sliding_window', sample_plots=False, droplow=False, start=0,
        ...
        if method == 'effective_growth_rate':
            ...
            if method == 'sliding_window':
                for sample in self.samples.values():  # find max growth rate using sliding window
                    sample.calculate_growth_parameters(droplow=droplow)
                    sample.get_lag_parameters_MF()
                    sample.get_sat_parameters_MF()
            elif method == 'smooth_n_slide':  # fit spline then find max rate using sliding window
                for sample in self.samples.values():
                    sample.smooth_n_slide(droplow=droplow)
                    self.get_lag_parameters_MF()
                    self.get_sat_parameters_MF()
...
class Sample(object):  # create Sample class for attributes collected for each column of data
    ...
    def get_lag_parameters(self):
        ...
        log_data = list(self.log_data)
        lag_thresh = 0.05 # threshold for lag-line
        lag_ratio = 3. # difference in slope between lag and growth phase, used to be 4 
        # problem with some initial values being negative after subtracting blank
        try: 
            first_logOD = next(x for x in log_data[3:] if not np.isnan(x))  # gets first non-nan logOD value # we skip first 3-points for low reference, as they are off
        ...
        for i, p in enumerate(log_data):
            ...
            if first_logOD - lag_thresh <= p <= first_logOD + lag_thresh:
                low_ODs.append(p)
        ...
        if self.growth_rate > 0 and lag_line[0] < self.growth_rate/lag_ratio:
    ...
    def get_sat_parameters_MF(self):  # run sliding window for corrected data rates
        ...
        if self.growth_rate > 0 or self.growth_rate is None:
            ...
            if last_rate < max_rate/4:
                ...
                if self.sat_index is not None and self.sat_index > max_index:
                    self.sat_time = self.elapsed_time[self.sat_index]
    ...
    def plot_growth_parameters(self, show=True, save=False, folder=None):
        ...
        if self.experiment.method == 'effective_growth_rate':
        ...
        if save:
            if folder is None: folder = self.experiment.out_dir
            if self.experiment.method == 'effective_growth_rate':
                plot_file = self.name + 'eff_gr_plot.png' #.png
            else:
                plot_file = self.name + '_plot.png' #.png
...
def analyze_experiment(
        ...
        sample_plots=False, droplow=False, start=0, end=None, saturation=False, correction=None, denoising=0):
    experiment = Experiment(data_file, plate_layout, blank, blank_file, out_dir, window_size, correction, denoising)
